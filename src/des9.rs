// 9front-compatible DES implementation
//
// This is a direct port of 9front's des.c which uses non-standard
// byte interleaving in the initial/final permutation.
// Required for p9sk1 authentication compatibility with Plan 9/9front.
...
const KEY_SH: [usize; 16] = [1, 1, 2, 2, 2, 2, 2, 2, 1, 2, 2, 2, 2, 2, 2, 1];

/// Expand 7-byte Plan 9 DES key to 8-byte using 9front's parity table
pub fn des56to64(key7: &[u8; 7]) -> [u8; 8] {
    let hi = ((key7[0] as u32) << 24) | ((key7[1] as u32) << 16) | ((key7[2] as u32) << 8) | (key7[3] as u32);
    let lo = ((key7[4] as u32) << 24) | ((key7[5] as u32) << 16) | ((key7[6] as u32) << 8);

    [
        PARITY_TABLE[((hi >> 25) & 0x7f) as usize],
        PARITY_TABLE[((hi >> 18) & 0x7f) as usize],
        PARITY_TABLE[((hi >> 11) & 0x7f) as usize],
        PARITY_TABLE[((hi >> 4) & 0x7f) as usize],
        PARITY_TABLE[(((hi << 3) | (lo >> 29)) & 0x7f) as usize],
        PARITY_TABLE[((lo >> 22) & 0x7f) as usize],
        PARITY_TABLE[((lo >> 15) & 0x7f) as usize],
        PARITY_TABLE[((lo >> 8) & 0x7f) as usize],
    ]
}

/// 9front's DES key schedule generation
fn des_key_setup(key: &[u8; 8]) -> [u32; 32] {
    let v0 = (key[0] as u32) | ((key[2] as u32) << 8) | ((key[4] as u32) << 16) | ((key[6] as u32) << 24);
    let v1 = (key[1] as u32) | ((key[3] as u32) << 8) | ((key[5] as u32) << 16) | ((key[7] as u32) << 24);

    let mut left = ((v0 >> 1) & 0x40404040)
        | ((v0 >> 2) & 0x10101010)
        | ((v0 >> 3) & 0x04040404)
        | ((v0 >> 4) & 0x01010101)
        | ((v1 >> 0) & 0x80808080)
    ...
    key_comp_perm(left, right)
}

fn key_comp_perm(mut left: u32, mut right: u32) -> [u32; 32] {
    let mut ek = [0u32; 32];
    let mut ek_idx = 0;

    for i in 0..16 {
        left = ((left << KEY_SH[i]) | (left >> (28 - KEY_SH[i]))) & 0xfffffff0;
        right = ((right << KEY_SH[i]) | (right >> (28 - KEY_SH[i]))) & 0xfffffff0;
...
/// 9front's DES block cipher with byte interleaving
fn block_cipher(key: &[u32; 32], text: &mut [u8], offset: usize, decrypting: bool) {
    // Initial permutation with 9front's byte interleaving
    let mut v0 = (text[offset] as u32)
        | ((text[offset + 2] as u32) << 8)
        | ((text[offset + 4] as u32) << 16)
        | ((text[offset + 6] as u32) << 24);
    let mut left = (text[offset + 1] as u32)
        | ((text[offset + 3] as u32) << 8)
        | ((text[offset + 5] as u32) << 16)
        | ((text[offset + 7] as u32) << 24);

    let mut right = (left & 0xaaaaaaaa) | ((v0 >> 1) & 0x55555555);
    ...
    for _ in 0..8 {
        v0 = key[key_idx as usize] ^ ((right >> 1) | (right << 31));
        ...
        let v1 = key[(key_idx + 1) as usize] ^ ((right << 3) | (right >> 29));
        ...
        let v1 = key[(key_idx + 1) as usize] ^ ((left << 3) | (left >> 29));
    ...
    let mut v1 = (left & 0xaaaaaaaa) | ((right >> 1) & 0x55555555);

    v1 = ((v1 << 6) & 0x33003300) | (v1 & 0xcc33cc33) | ((v1 >> 6) & 0x00cc00cc);
    v1 = ((v1 << 12) & 0x0f0f0000) | (v1 & 0xf0f00f0f) | ((v1 >> 12) & 0x0000f0f0);
    ...
    text[offset + 1] = v1 as u8;
    text[offset + 3] = (v1 >> 8) as u8;
    text[offset + 5] = (v1 >> 16) as u8;
    text[offset + 7] = (v1 >> 24) as u8;
}

/// Plan 9's non-standard DES encryption with 7-byte stride
/// Uses 9front's exact block_cipher implementation
pub fn plan9_encrypt(key: &[u8; 7], data: &mut [u8]) {
    if data.len() < 8 {
        return;
    }

    let des_key = des56to64(key);
...
mod tests {
    ...
    #[test]
    fn test_des56to64() {
        // Test key expansion produces valid parity
        let key7 = [0xF4, 0xF2, 0x9C, 0x1E, 0x93, 0xCD, 0x68];
        let key8 = des56to64(&key7);

        // Each byte should have odd parity
        for byte in key8 {
            assert_eq!(byte.count_ones() % 2, 1, "Byte {:02x} should have odd parity", byte);
        }
    }

    #[test]
    fn test_encrypt_decrypt_roundtrip() {
        let key = [0xF4, 0xF2, 0x9C, 0x1E, 0x93, 0xCD, 0x68];
        let original = b"Hello, Plan 9 world! Testing.";
    ...
    fn test_known_encryption() {
        // Test against known 9front output
        // Key derived from "test1234" password
        let key = [0xF4, 0xF2, 0x9C, 0x1E, 0x93, 0xCD, 0x68];

        // Verify key expansion matches 9front
        let key8 = des56to64(&key);
        assert_eq!(key8, [0xF4, 0x79, 0xA7, 0x83, 0xE9, 0x9E, 0x34, 0xD0]);
    }
}
